...
long int hog_word_embeddings_model_replication_per_node_avg_all() {
...
#pragma omp parallel for reduction(+:C_A,C_B)
	...
	//Model averaging
	if (i % AVERAGING_FREQ == 0 && n_numa_nodes >= 2) {
	  average_n_models(n_numa_nodes, model, N_NODES, K);
	}
...
long int hog_word_embeddings_model_replication_per_node_add() {
