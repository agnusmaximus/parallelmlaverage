...
def cache_partition_rec_helper(tmp_dir, input_file_name, partition_list, output_file_name, depth):
    ...
    for fname in files:
        ...
        cache_partition_rec_helper(tmp_dir, remapped_filename, partition_list, result_file_next_level, depth-1)
...
def cache_partition_recursive(input_file_name, n_partitions, output_file_name):
    ...
    os.makedirs(tmp_dir)
    partition_list = [10, 10, 10, 10, 10][::-1]
    cache_partition_rec_helper(tmp_dir, input_file_name, partition_list, output_file_name, len(partition_list)-1)
